...
inline vptr align(const vptr addr, const u8 alignment)
{
...
template<typename T>
inline T* align_ptr(T* ptr, const u8 alignment)
{
    const auto addr = reinterpret_cast<vptr>(ptr);
...
inline void* alloc_aligned(const u64 size, const u8 alignment)
{
...
inline void* realloc_aligned(void* memory, const u64 size, const u8 alignment)
{
    if (!memory)
    {
        return nullptr;
    ...
    u64 original_size = reinterpret_cast<u64*>(static_cast<u8*>(memory) - header_size - 1)[0];
